"""
Official code from  Relative Positional Encoding for Transformers with Linear Complexity by Antoine Liutkus, Ondřej Cífka, Shih-Lun Wu, Umut Şimşekli, Yi-Hsuan Yang and Gaël Richard
"""
...
class SineSPE(nn.Module):
    ...
    def __init__(
        ...
        for param in ["freqs", "offsets", "gains"]:
            self.register_parameter(
                param, nn.Parameter(torch.randn(num_heads, in_features, num_sines))
            )

        # normalize the gains
        self.gains.data[...] /= torch.sqrt(self.gains.norm(dim=-1, keepdim=True)) / 2.0
    ...
    def forward(self, shape, num_realizations=None):
        ...
        freqs = torch.sigmoid(self.freqs[:, :, None, :]) / 2.0

        phases_q = (
            2 * math.pi * freqs * indices[None, None, :, None]
            + self.offsets[:, :, None, :]
        )
        ...
        # now upsample it
        gains = torch.stack((gains, gains), dim=-1).view(
    ...
    def get_posattn_matrix(self, max_len=2048):
        ...
        print(
            "[get posattn matrix] Omega_q: {}, lambda: {}, Omega_k: {}".format(
        ...
        omega_q_mult_gains_squared_diag = torch.einsum(
            "ihdmk, hdku -> ihdmu", omega_q, gains_squared_diag
        )
        pos_attn_matrices = torch.einsum(
            "ihdmk, ihdnk -> ihdmn", omega_q_mult_gains_squared_diag, omega_k
        )
        print("[get posattn matrix] pos_attn: {}".format(pos_attn_matrices.size()))
...
class ConvSPE(nn.Module):
    ...
    def forward(self, shape, num_realizations=None):
        ...
        for dim in range(len(shape)):
            ...
            indices = [
                slice(batchsize * num_realizations),
                slice(self.num_heads * self.in_features),
            ] + [
                slice(k, k + s, 1),
            ]
            qbar = qbar[indices]
    ...
    def get_posattn_matrix(self, shape, num_realizations=None):
        ...
        print(
            "[get posattn matrix] Qbar: {}, Kbar: {}".format(qbar.size(), kbar.size())
        )

        # get (num_heads * keys_dim) attention maps, each of size (max_len, max_len)
        pos_attn_matrices = torch.einsum("rdm, rdn -> dmn", qbar, kbar)
        print("[get posattn matrix] pos_attn: {}".format(pos_attn_matrices.size()))
...
class SPEFilter(nn.Module):
    ...
    def __init__(
        ...
        if gated:
            ...
            self.register_parameter("gate", nn.Parameter(torch.randn(code_shape)))
    ...
    def forward(
        ...
        if len(code_size) != len(query_size) or torch.any(
        ...
        if self.gated:
            ...
            gating_noise = (
                torch.randn(
                    self.code_shape + (num_realizations,), device=queries.device
                )
            ...
            qbar = torch.sqrt(1.0 - gate) * qbar + torch.sqrt(gate) * gating_noise
            kbar = torch.sqrt(1.0 - gate) * kbar + torch.sqrt(gate) * gating_noise
