...
def topological_dot_product_attention_weights(
    ...
    dist_index = (
        (grid_i[:, None, None, None] - grid_i[None, None, :, None] + nb_x_patches)
        * 2
        * nb_y_patches
        + grid_j[None, :, None, None]
        - grid_j[None, None, None, :]
        + nb_y_patches
    )
    ...
    toeplitz_mask = toeplitz_mask.reshape(toeplitz_params.shape[0], -1)[
        :,
        dist_index,
    ]
    ...
    toeplitz_mask = toeplitz_mask.reshape(
        (
            toeplitz_params.shape[0],
            nb_x_patches * nb_y_patches,
            nb_x_patches * nb_y_patches,
        )
    )
    toeplitz_mask = F.pad(
        toeplitz_mask, (0, 0, 1, 0, 0, 0), value=1.0
    )  # needed to add in the CLS token
    toeplitz_mask = F.pad(
        toeplitz_mask, (1, 0, 0, 0, 0, 0), value=1.0
    )  # needed for the CLS token in VIT
    ...
    attn_weights = nn.Dropout(p=dropout_rate)(attn_weights)

    return attn_weights


def dot_product_attention_2d_vis(
    query,
    key,
    value,
    toeplitz_params,
...
class MultiHeadDotProductAttention_forViT(nn.Module):
    ...
    def __init__(
        ...
        attention_fn=dot_product_attention_2d_vis,
        nb_x_patches=0,
        nb_y_patches=0,
    ):
        super().__init__()
        ...
        self.nb_x_patches = nb_x_patches
        self.nb_y_patches = nb_y_patches

        self.dropout = nn.Dropout(p=self.dropout_rate)
        self.q_dense = functools.partial(
            nn.Linear,
        )
        self.k_dense = functools.partial(
            nn.Linear,
        )
        self.v_dense = functools.partial(
            nn.Linear,
        )
        self.out_dense = functools.partial(
            nn.Linear,
        )
    ...
    def forward(
        ...
        query, key, value = (
            self.q_dense(inputs_q.shape[-1], self.num_heads * head_dim)(inputs_q),
            self.k_dense(inputs_kv.shape[-1], self.num_heads * head_dim)(inputs_kv),
            self.v_dense(inputs_kv.shape[-1], self.num_heads * head_dim)(inputs_kv),
        )
        ...
        toeplitz_params = nn.Parameter(
            torch.ones(
                query.shape[-2],
                4
                * self.nb_x_patches
                * self.nb_y_patches,  # must be a multiple of 4 for the patch dims
            )
        )
        ...
        out = self.out_dense(x.shape[-1])(x)
